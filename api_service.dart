...
class ApiService {
  ...
  // ✅ Token stocké en mémoire (pas besoin de shared_preferences)
  ...
  static Future<Map<String, dynamic>> createTrip({
    ...
    // ✅ Mapping Flutter → valeurs acceptées par Laravel
    ...
    try {
      final response = await http.post(
        ...
        headers: _headers,
        body: jsonEncode({
          "vehicle_type": mappedVehicle, // ✅ "driving" | "cycling" | "walking"
          "start_lat": startLat,
          "start_lng": startLng,
          "status": "pending",
        }),
  ...
  // ══════════════════════════════════════════
  //  AJOUTER UNE STATION
  //  POST /api/trips/{trip_id}/stations
  // ══════════════════════════════════════════
  ...
  // ══════════════════════════════════════════
  //  OPTIMISER LE TRIP (TSP — collègue)
  //  POST /api/trips/{trip_id}/optimize
  //  Body: { "profile": "driving" }
  //  Réponse attendue: { "stations": [...], "order": [...] }
  // ══════════════════════════════════════════
  static Future<Map<String, dynamic>> optimizeTrip({
    ...
    try {
      ...
      ).timeout(const Duration(seconds: 60)); // TSP peut prendre du temps
      ...
      if (response.statusCode == 200) {
        return {
          "success": true,
          // ✅ stations dans l'ordre optimal retourné par le TSP
  ...
  //  PATCH /api/stations/{station_id}/status
  // ══════════════════════════════════════════
  ...
  // ══════════════════════════════════════════
  //  HELPER — parse JSON + détecte HTML ngrok
  // ══════════════════════════════════════════
  static Map<String, dynamic> _parseResponse(http.Response response) {
    try {
      ...
      if (!contentType.contains("application/json")) {
        ...
        return {
          "message":
          "Serveur ngrok non joignable ou retourne du HTML. Vérifiez le tunnel."
        };
      }
